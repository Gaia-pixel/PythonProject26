...
# 3 – Cammino con somma dei pesi ≤ soglia (massimo possibile entro un limite)
# ad esempio budget
import networkx as nx
import copy
...
class Model:
    ...
    def getCamminoSottoSoglia(self, partenza, soglia):
        self._bestPath = []
        parziale = [partenza]
        self._ricorsione_sotto_soglia(parziale, soglia)
        return self._bestPath

    def _ricorsione_sotto_soglia(self, parziale, soglia_rimasta):
        if self._calcolaPeso(parziale) > self._calcolaPeso(self._bestPath):
            self._bestPath = copy.deepcopy(parziale)

        ultimo = parziale[-1]
        for vicino in self._graph.successors(ultimo):
            peso = self._graph[ultimo][vicino]['weight']
            if vicino not in parziale and peso <= soglia_rimasta:
                parziale.append(vicino)
                self._ricorsione_sotto_soglia(parziale, soglia_rimasta - peso)
                parziale.pop()
...
# 3B ALTERNATIVA 10 - Cammino con durata totale ≤ MAX e valore massimo caso specifico LAB14
# SE "Seleziona il miglior sottoinsieme di nodi con durata totale ≤ D,
...
# 4 - Cammino di lunghezza esattamente N archi
# Non restituisce i cammini, ma solo il numero totale di quelli validi.
import networkx as nx
...
class Model:
    ...
    def getCamminoLunghezzaEsatta(self, partenza, N):
        self._count = 0
        parziale = [partenza]
        self._ricorsione_lunghezza(parziale, N)
        return self._count

    def _ricorsione_lunghezza(self, parziale, N):
        # Se il cammino ha esattamente N archi, lo conto
        if len(parziale) - 1 == N:
            self._count += 1
            return
...
# 4B - ottenere la lista completa dei cammini di lunghezza
import networkx as nx
import copy
...
class Model:
    ...
    def getCamminiLunghezzaEsatta(self, partenza, N):
        self._soluzioni = []
        parziale = [partenza]
        self._ricorsione_lunghezza(parziale, N)
...
# 5 - Ciclo di lunghezza esattamente N che torna al nodo iniziale
import networkx as nx
import copy
...
class Model:
    ...
    def getCicliConVincoli(self, partenza, N):
        self._cicli = []
        parziale = [partenza]
        self._ricorsione_ciclo(parziale, N)
        return self._cicli

    def _ricorsione_ciclo(self, parziale, N):
        if len(parziale) == N + 1:
            # Se ho N archi e torno alla partenza, è un ciclo valido
            if parziale[-1] == parziale[0]:
                self._cicli.append(copy.deepcopy(parziale))
            return

        ultimo = parziale[-1]
        for vicino in self._graph.successors(ultimo):
            # Posso tornare alla partenza solo all’ultimo passo
...
# 6 - Componente connessa di un nodo senza ricorsione
import networkx as nx

class Model:

    def __init__(self, grafo: nx.Graph):
        # ⚠️ Usa un grafo NON orientato per componenti connesse
        self._graph = grafo

    def getComponenteConnessaDiNodo(self, nodo):
        if nodo not in self._graph:
            return set()
        # NetworkX ha una funzione pronta per questo!
        return nx.node_connected_component(self._graph, nodo)
# 6B - con ricorsione
